...
The number of milliseconds of reaction time that each person in the study demonstrated.
...
`ggplot(sleep, aes(y = rxn_time, x = day, color = Subject))`

#### Part c {.unnumbered}

How can you obtain the dataset you sketched in part b? I can use `pivot_longer()`
...
-   Besides putting each argument on a different line for readability and storing the results, what changed in the code? `values_to` was used.
-   How did this impact how the values are recorded in the `day` column? This resulted in the day column switching to just the numerical values contained in the previous variable.

#### Part c {.unnumbered}

Using `sleep_long`, construct a line plot of reaction time vs day for each subject. This will look goofy no matter what you do. Why? HINT: look back at `head(sleep_long)`. What class or type of variables are `Subject` and `day`? What do we *want* them to be? We want day to be a numerical variable and subject to be a character variable.
...
-   What's the general relationship between reaction time and sleep? Generally, reaction time increased with more days of sleep deprivation.
-   Is this the same for everybody? What differs? Some people experience this effect much more strongly than others, and this may depend on how many hours of sleep they were getting regularly before the study began or other factors as well. People's initialy reaction time vary dramatically as well.
...
Adjust the code below. What don't you like about the column labels? The column headers have no indicator of what they represent now.
